...
export const Layout: React.FC<LayoutProps> = ({ children, currentTab, setCurrentTab }) => {
  ...
  return (
    <div className="min-h-screen bg-bloomberg-bg text-bloomberg-text-primary flex flex-col font-mono selection:bg-bloomberg-gold/30 selection:text-white">
      ...
      {/* ── DANGER BANNER ─────────────────────────────────────────────── */}
      {isLocked && (
        <div className="bg-bloomberg-red/10 border-b border-bloomberg-red/40 px-5 py-2 flex items-center justify-between text-[10px] animate-fade-in">
          <div className="flex items-center space-x-2.5">
            ...
            <span className="font-bold text-bloomberg-red-light tracking-widest uppercase">
              SESSION VERROUILLÉE — 2 STOP LOSS CONSÉCUTIFS
            </span>
            <span className="hidden md:inline text-bloomberg-red/70 italic">
              "{lock?.lock_reason || 'La discipline avant tout.'}"
            </span>
          </div>
          <span className="text-bloomberg-red-light font-bold uppercase tracking-widest border border-bloomberg-red/30 px-2 py-0.5 text-[9px]">
      ...
      {/* ── TOPBAR ────────────────────────────────────────────────────── */}
      <header className="h-11 border-b border-bloomberg-border bg-bloomberg-surface flex items-center justify-between px-5 shrink-0 relative">
        {/* Top accent line */}
        ...
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-1.5 group">
            <div className="relative">
              ...
              <div className="absolute inset-0 blur-sm text-bloomberg-gold opacity-0 group-hover:opacity-60 transition-opacity duration-300">
          ...
          <span className="text-[8px] text-bloomberg-text-muted border border-bloomberg-border px-1.5 py-0.5 tracking-widest">
            TERMINAL&nbsp;V2
          </span>
        </div>

        {/* Account selector + live indicator */}
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-2">
            <span className="text-[9px] text-bloomberg-text-secondary uppercase tracking-wider hidden sm:block">Compte :</span>
            <select
              value={activeAccountId || ''}
              onChange={(e) => setActiveAccountId(e.target.value || null)}
              className="bg-bloomberg-bg border border-bloomberg-border text-[10px] px-2.5 py-1 font-mono text-white focus:outline-none focus:border-bloomberg-gold cursor-pointer transition-colors hover:border-bloomberg-border-bright"
            >
              <option value="">[ TOUS LES COMPTES ]</option>
              {accounts.map((acc) => (
                <option key={acc.id} value={acc.id}>
                  {acc.name.toUpperCase()} ({acc.currency} {acc.balance.toLocaleString()})
                </option>
              ))}
            </select>
          </div>
          ...
          <button
            ...
            className="flex items-center gap-1.5 border border-bloomberg-border hover:border-bloomberg-red-light/50 hover:bg-bloomberg-red/10 text-bloomberg-text-secondary hover:text-bloomberg-red-light transition-all text-[10px] px-2.5 py-1 font-mono uppercase tracking-wider"
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        {/* ── SIDEBAR ───────────────────────────────────────────────── */}
        <aside className={`border-r border-bloomberg-border bg-bloomberg-surface flex flex-col justify-between shrink-0 transition-all duration-300 ease-out ${sidebarOpen ? 'w-52' : 'w-14'}`}>
                ...
                >
                  ...
                  {sidebarOpen && (
                    <div className="flex flex-col min-w-0">
                      ...
                      <span className="text-[8px] text-bloomberg-text-muted truncate hidden">
                        {item.label}
                      </span>
                    </div>
                  )}
        ...
        <main className="flex-1 overflow-y-auto bg-bloomberg-bg bg-grid relative">
          ...
          <div className="relative z-10 max-w-screen-2xl mx-auto p-6">
